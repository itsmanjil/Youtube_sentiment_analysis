...
def test_preprocessing():
    ...
    for comment in test_comments:
        ...
        reason = f"({metadata.get('filter_reason', 'none')})" if metadata['filtered'] else ""
...
def test_youtube_api():
    ...
    try:
        ...
        if metadata:
            print(f"✓ API working! Fetched: {metadata['title'][:50]}...")
            print(f"  Views: {metadata['view_count']:,}")
            print(f"  Likes: {metadata['like_count']:,}")
        else:
            print("✗ API returned no metadata")
...
def test_scraper():
    ...
    try:
        ...
        if comments:
            print(f"✓ Scraper working! Fetched {len(comments)} comments")
            print(f"  First comment: {comments[0]['text'][:50]}...")
        else:
            print("✗ Scraper returned no comments")
...
def main():
    ...
    if all(checks.values()):
        ...
        print("⚠️  Core features working. Fix warnings above for full functionality.")
    else:
